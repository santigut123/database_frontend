    ...
    if (!Array.isArray(question)) {
      ...
      return (
        <Stack key={index}>
          ...
          <Text>Results:</Text>
          {question.results.map((result: any, index: any) => (
            <Text key={index}>{result.answer}</Text>
          ))}
        </Stack>
      );
    }
...
export default function yourSurvey() {
  ...
  async function getQuestions(input_survey_id: any) {
    let { data, error } = await supabase.rpc('getsurveyquestions', {
      input_survey_id,
    });
    console.log(data);
    for (let i = 0; i < data.length; i++) {
      let input_question_id = data[i].question_id;
      let response = await supabase.rpc('getsurveyanswers', {
        input_question_id,
      });
